...
const FabricCanvas = forwardRef<FabricCanvasRef, FabricCanvasProps>(
  ({ width, height, layers, selectedLayerId, onLayerUpdate, onLayerSelect }, ref) => {
    ...
    useEffect(() => {
      ...
      logger.info('FabricCanvas', 'Fabric.js canvas initialized', { width, height });
    ...
    useEffect(() => {
      ...
      logger.debug('FabricCanvas', 'Canvas dimensions updated', { width, height });
    }, [width, height]);
    ...
    const createFabricObject = (layer: Layer, fabricCanvas: fabric.Canvas, index?: number) => {
      ...
      if (obj) {
        ...
        logger.debug('FabricCanvas', 'Created object', { layerId: layer.id, type: layer.type, index });
      }
    };
